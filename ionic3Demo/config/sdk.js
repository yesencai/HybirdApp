...
var EV_Connect             = 1;
var EV_Disconnect          = 2;
var EV_LoginSuccess        = 3;
var EV_LoginFail           = 4;
var EV_GetTopicSuccess     = 5;
var EV_GetTopicFail        = 6;
var EV_ReleaseTopicSuccess = 7;
var EV_RecvBeatPack        = 8;

//当事件为EV_Disconnect时的数据
var EVDATA_CONN_LOST = "1"; //掉线了
var EVDATA_CONN_CLOSE = "2"; //客户端主动断开连接

var LOGTYPE_INFO  = 0; //普通提示型日志
var LOGTYPE_ERROR = 1; //错误提醒型日志

var SRCTYPE_SERVER    = 0;     //从应用服务器发过来的
var SRCTYPE_DEVCLI    = 1;     //从其它设备发送来的

var DSTTYPE_SERVER     = 2;     //发送给应用服务器，这里的值有两种，0和2，
                                 //当服务器是使用普通的设备来实现时填写2，
                                 //当服务器是使用消息队列来实现时填写0
var DSTTYPE_DEVCLI    = 1;     //发送给其它设备
...
//其它说明：
//
function SdkInit(LogEchoCallBack, EventCallBack, RecvCallBack, DeveloperID, CustomData)
{
...
//其它说明：
//
function SdkSendStr(DstType, DstID, StrPack)
{
    ...
    SendLogText("发送数据包 DstType="+DstType+", DstID="+DstID+", Data="+mJsonStr+", Topic="+mTopic);
